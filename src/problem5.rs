...
/// mode. Right now, your ship computer already understands parameter mode 0,
...
/// digit of the value, that is, the opcode is the rightmost two digits of the
...
/// parts of the Intcode computer, like parameter modes, function
/// correctly. For each test, it will run an output instruction indicating how
/// far the result of the test was from the expected value, where 0 means the
...
/// halt. This final output isn't an error; an output followed immediately by a
...
pub fn run() -> utils::ProblemResult<()> {
    ...
    println!("Part 1");
    println!("------");
    let mut io = StaticIO::new(vec![1]);

    match program.run(&mut io) {
        Ok(_) => {
