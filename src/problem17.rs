...
mod cell {
    ...
    const HASH: i64 = '#' as i64;
    const UP: i64 = b'^' as i64;
    const DOWN: i64 = 'v' as i64;
    const LEFT: i64 = '<' as i64;
    const EMPTY: i64 = '.' as i64;
    const RIGHT: i64 = '>' as i64;
    ...
    impl Into<char> for Cell {
        fn into(self) -> char {
            match self {
                Cell::Empty => '.',
                Cell::Robot(Direction::North) => '^',
                Cell::Robot(Direction::South) => 'v',
                Cell::Robot(Direction::West) => '<',
                Cell::Robot(Direction::East) => '>',
                Cell::Scaffold => '#',
            }
        }
    }
    ...
    impl TryFrom<i64> for Cell {
        type Error = BadCell;
        fn try_from(i: i64) -> Result<Cell, BadCell> {
            let res = match i {
                HASH => Cell::Scaffold,
                EMPTY => Cell::Empty,
                UP => Cell::Robot(Direction::North),
                DOWN => Cell::Robot(Direction::South),
                LEFT => Cell::Robot(Direction::West),
                RIGHT => Cell::Robot(Direction::East),
                _ => return Err(BadCell(i)),
            };
    ...
    impl Grid<Cell> {
        pub fn from_program_output(v: Vec<i64>) -> Result<Grid<Cell>, BadCell> {
            let mut row: i64 = 0;
            let mut col: i64 = 0;
            ...
            for num in v {
                match num {
                    ...
                    _ => {
                        cells.insert((col, row), Cell::try_from(num)?);
                        col += 1;
                    }
                }
            }
        ...
        pub fn is_empty(&self, c: Coord) -> bool {
            self.get(&c) == Cell::Empty
        }

        pub fn as_move_sequence(&self) -> Vec<Move> {
            let mut out: Vec<Move> = Vec::new();
            let (mut loc, mut direction) = self.find_robot();

            loop {
                if self.is_empty(loc + direction) {
                    let next_turn = [Turn::CW, Turn::CCW].iter().find_map(|&turn| {
                        let test = loc + turn.apply(direction);
                        if !self.is_empty(test) {
                            Some(turn)
                        } else {
                            None
                        }
                    ...
                    match next_turn {
                        Some(t) => {
                            println!("loc={:?}, direction={:?}, turn={:?}", loc, direction, t);
                            direction = t.apply(direction);
                            out.push(Move::Rotate(t));
                            out.push(Move::Forward(1));
                        }
                        None => {
                            return out;
                        }
        ...
        fn find_robot(&self) -> (Coord, Direction) {
            self.iter()
                .find_map(|(coord, tile)| match tile {
                    Cell::Robot(d) => Some((*coord, *d)),
                    _ => None,
                })
                .expect("No robot on grid!")
        }
    }

    #[derive(Debug, Clone, Copy)]
    pub struct BadCell(i64);

    impl fmt::Display for BadCell {
    ...
    impl Move {
        pub fn increment(&mut self) {
            match self {
                Move::Forward(i) => {
                    *self = Move::Forward(*i + 1);
                }
                _ => panic!("Can't extend non-forward move: {:?}", self),
            }
        }
    }
}
...
pub fn run() -> ProblemResult<()> {
    let program = Program::for_problem(17)?;

    let mut io = StaticIO::new(vec![]);
    program.run(&mut io)?;

    let results = io.outputs();
    let grid = Grid::<Cell>::from_program_output(results)?;
    println!("{}", grid.render());

    // Part 1
    {
        let intersections = get_intersections(&grid);
        let total: i64 = intersections.map(|(x, y)| x * y).sum();
        println!("Total: {}", total);
    }

    // Part 2
    {
        let sequence = grid.as_move_sequence();
        println!("{:?}", sequence);
    }
...
fn get_intersections<'a>(grid: &'a Grid<Cell>) -> impl Iterator<Item = &'a Coord> {
    grid.iter()
        ...
        // Intersections are locations with at least 3 nonempty neighbors.
        .filter(move |coord| {
